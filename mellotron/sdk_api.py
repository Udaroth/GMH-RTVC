...
from mellotron import inference as mellotron, hparams
...
def tts_sdk(text, **kwargs):
    ...
    '''
    TODO: tts_sdk_base will be returning mels instead of wav
    For each set of mels that are returned, we should run the silence threhsold logic
    Then appending it into specs
    Actually seems pretty straight forward, let's do it and see if anything goes wrong.
    '''

    for text_split in text_split_lst:
        logger.info(f'Synthesizing: {text_split}')
        mels = tts_sdk_base(text_split, **kwargs)

        # First we need to perform a detach
        mels.detach().cpu().numpy()
        # Then we'll use the same trim silence logic
        print(mels)
        for m in mels:
            # Trim silence from end of each spectrogram
            while np.max(m[:, -1]) < _hparams.tts_stop_threshold:
                m = m[:, :-1]
            specs.append(m)

        # Ok at this point the specs should contain what we need, I think we can pass it back
        # wav_lst.append(wav)
...
def tts_sdk_base(text, speaker='Aiyue', audio='14', output='', **kwargs):
    ...
    print(mels_postnet)
    print(mels_postnet.device)


    # TODO: We need to modify this section to return the mels instead of wav
